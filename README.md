...
| Skill                                           | Description                                                                                                                                                                                                                                                                          |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [panel-review](./skills/panel-review)           | Fan a code review out to multiple local CLI agents (codex, claude, opencode) in parallel — each driven through the `dash-p` CLI — and amalgamate their findings                                                                                                                      |
...
| [pr-review-tab](./skills/pr-review-tab)         | The tab-lifecycle wrapper for a review spawned by `review-prs` — run `auto-review`, close the enclosing Herdr/cmux tab on approval, and with `--babysit` loop `recheck-pr` on an interval until the PR is approvable, then close the tab                                             |
...
| [write-pull-request](./skills/write-pull-request)   | Write and open a PR a reviewer can skim: an imperative plain-language title and a body that leads with a short Changes list before Why (problem or goal, reason, rejected alternative, cost), How to verify, and Risk. Derives both from the diff and commits, honors a repo PR template, lints the result with `pr-lint.sh`, and is what `ship` delegates PR-writing to. Adapted from [tjcages/skills](https://github.com/tjcages/skills/tree/main/pr-writing) |
