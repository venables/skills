...
| [write-pull-request](./skills/write-pull-request) | Open a PR a human can skim: a plain-language title and a body that leads with a short Changes list before the reasoning (Problem / Solution, or a Goal for net-new features) and Testing. Derives both from the diff and commits, honors a repo PR template, and is what `ship` delegates PR-opening to |
