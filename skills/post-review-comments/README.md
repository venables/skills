...
- Posts each selected finding as a **standalone inline comment** via `POST /pulls/{N}/comments` — no review wrapper, no "X left a review" timeline entry. Calls are sequenced HIGH → MEDIUM → LOW so notifications arrive in priority order.
...
- **No provenance footer on PR comments.** Panelist attribution stays in the triage transcript and chat, not on the PR.
