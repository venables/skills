...
5. **Open the PR** using the `write-pull-request` skill. It writes the imperative plain-language title and the scannable one-screen body (an overview line, a short `Changes` list, then `Why`, `Decisions`, `How to verify`, and `Risk`), checks for a repo PR template, lints the result, and opens the PR. Reference the tracker ticket when one exists, fold plan findings into the body rather than committing a `plans/` folder, and give multiple independent changes their own PRs, not one omnibus.
