...
5. **Open the PR** using the `create-pull-request` skill. It writes the plain-language title and the skimmable body (a `Changes` list first, then Problem, Solution, Testing), checks for a repo PR template, and opens the PR. Reference the tracker ticket when one exists, fold plan findings into the body rather than committing a `plans/` folder, and give multiple independent changes their own PRs, not one omnibus.
