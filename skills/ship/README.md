...
- **Opens the PR via `write-pull-request`.** Delegates to that skill for the imperative plain-language title and the scannable one-screen body (an overview line, a short Changes list, then Why, Decisions, How to verify, and Risk), the PR-template check, the lint, and the `gh pr create`. References the tracker ticket when one exists, and gives multiple independent changes their own PRs rather than one omnibus.
...
- **It delegates to other skills.** The panel gate calls `panel-review-loop`, opening the PR calls `write-pull-request`, and comment handling calls `pr-comment-handler`, so install those too for the full workflow.
