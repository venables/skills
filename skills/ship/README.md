...
- **Opens the PR via `write-pull-request`.** Delegates to that skill for the imperative plain-language title and the skimmable body (a Changes list, then Why, How to verify, and Risk), the PR-template check, the lint, and the `gh pr create`. References the tracker ticket when one exists, and gives multiple independent changes their own PRs rather than one omnibus.
