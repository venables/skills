...
- **It delegates to other skills.** The panel gate calls `panel-review-loop`, opening the PR calls `create-pull-request`, and comment handling calls `pr-comment-handler`, so install those too for the full workflow.
