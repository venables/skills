...
Write and open a GitHub pull request a reviewer can actually skim: a plain-language imperative title, and a body that leads with a short Changes list before Why, How to verify, and Risk. The reusable PR-writing procedure the `ship` playbook delegates to, usable on its own when you just want a PR written and opened.
...
Title: imperative (`If applied, this PR will ___`), repo prefix kept, target 50 chars after it, hard limit 72, no period, no emoji, no ticket ID or file path carrying the meaning. Body: `Changes` (one short line each), `Why` (the problem or goal, the reason for this solution, the rejected alternative, the cost), `How to verify` (numbered steps with the expected result), `Risk` (failure mode and rollback), `Closes ABC-123`. Active voice, present tense, 25 words max per sentence, the change is the actor (never `we`), no `simply` / `just` / `leverage` / `This PR ...`.
...
- **Leads the body with a Changes list.** One short line per change, so a reviewer sees the shape of the diff before reading `Why` (problem or goal, reason, rejected alternative, cost), `How to verify` (numbered steps and what was actually run), and `Risk` (failure mode and rollback).
...
- **The repo template wins.** If `.github/pull_request_template.md` is present, its sections take precedence over the Changes / Why / How to verify / Risk default. The title rules and language contract still apply.
